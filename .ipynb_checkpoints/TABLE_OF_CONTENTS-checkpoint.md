# Table of Contents

---

## Front Matter

| Section | Page |
|---------|------|
| Preface | ___ |
| How to Use This Book | ___ |
| List of Figures | ___ |
| List of Tables | ___ |

---

## Part I: Foundations

### Chapter 1: Introduction
| Section | Page |
|---------|------|
| 1.1 Why Fair Lending Matters | ___ |
| 1.2 The Regulatory Landscape | ___ |
| 1.3 What You'll Build | ___ |
| 1.4 Prerequisites and Setup | ___ |
| 1.5 How This Book Is Different | ___ |
| Key Takeaways | ___ |

### Chapter 2: Data Foundations
| Section | Page |
|---------|------|
| Executive Summary | ___ |
| 2.1 Understanding Data Quality | ___ |
| 2.2 Building the Data Pipeline | ___ |
| 2.3 Documentation and Auditability | ___ |
| 2.4 Connecting to Business Context | ___ |
| Key Takeaways | ___ |
| Teaching Notes | ___ |

**Figures:**
- Figure 2.1: Data Quality Issues (Completeness, Duplicates, Date Formats)

**Tables:**
- Table 2.1: The Five Dimensions of Data Quality
- Table 2.2: Regulatory Requirements for Data Management
- Table 2.3: Data Quality Standards by Use Case
- Table 2.4: Chapter 2 Self-Assessment Rubric
- Table 2.5: Key Terms: Data Foundations

---

## Part II: Model Development

### Chapter 3: Building the Credit Model
| Section | Page |
|---------|------|
| Executive Summary | ___ |
| 3.1 The Credit Modeling Challenge | ___ |
| 3.2 Building the Baseline Model | ___ |
| 3.3 The Honest Failure: When Models Break | ___ |
| 3.4 Model Improvement and Tuning | ___ |
| 3.5 Model Explainability with SHAP | ___ |
| Key Takeaways | ___ |
| Teaching Notes | ___ |

**Figures:**
- Figure 3.1: Confusion Matrix - SMOTE Model
- Figure 3.2: Top 10 Most Important Features (Logistic Regression)
- Figure 3.3: ROC Curve - Credit Default Model
- Figure 3.4: Precision-Recall Curve - Credit Default Model
- Figure 3.5: Top 10 Features - Random Forest
- Figure 3.6: Top 10 Features - XGBoost
- Figure 3.7: Model Comparison (Precision, Recall, ROC-AUC)
- Figure 3.8: Precision-Recall Trade-off by Decision Threshold
- Figure 3.9: Probability Distributions by Dataset
- Figure 3.10: Calibration Curves by Dataset
- Figure 3.11: SHAP Summary Plot
- Figure 3.12: SHAP Dependence Plots
- Figure 3.13: SHAP Waterfall - Highest Probability Prediction
- Figure 3.14: SHAP Waterfall - Correct Non-Default
- Figure 3.15: SHAP Waterfall - Missed Default (False Negative)
- Figure 3.16: SHAP Waterfall Examples - Combined View

**Tables:**
- Table 3.1: What Makes Credit Models Different
- Table 3.2: Class Imbalance Handling Approaches
- Table 3.3: Model Performance Across Datasets
- Table 3.4: Key Terms: Credit Modeling

---

## Part III: Fairness and Compliance

### Chapter 4: Fairness & Compliance
| Section | Page |
|---------|------|
| Executive Summary | ___ |
| 4.1 Understanding Algorithmic Fairness | ___ |
| 4.2 Measuring Bias in Your Model | ___ |
| 4.3 Bias Mitigation Techniques | ___ |
| 4.4 Production Monitoring for Fairness | ___ |
| 4.5 Regulatory Documentation | ___ |
| Key Takeaways | ___ |
| Common Pitfalls | ___ |
| Teaching Notes | ___ |

**Figures:**
- Figure 4.1: Fairness Dashboard (6-Panel)
- Figure 4.2: Calibration Comparison (Original vs. Platt vs. Isotonic)
- Figure 4.3: Accuracy vs. Fairness Trade-off

**Tables:**
- Table 4.1: Error Types in Credit Scoring
- Table 4.2: Mitigation Approach Comparison
- Table 4.3: Monitoring Alert Thresholds
- Table 4.4: Common Fairness Pitfalls
- Table 4.5: Key Terms: Fairness & Compliance

---

## Part IV: Looking Forward

### Chapter 5: Conclusion & Future Directions
| Section | Page |
|---------|------|
| Executive Summary | ___ |
| 5.1 Key Lessons Learned | ___ |
| 5.2 Emerging Regulations & Trends | ___ |
| 5.3 Building a Fairness-First Culture | ___ |
| 5.4 Final Thoughts | ___ |
| Teaching Notes | ___ |

**Tables:**
- Table 5.1: Mitigation Results Comparison
- Table 5.2: Mitigation Techniques: When They Act
- Table 5.3: Train/Validate/Test Purposes
- Table 5.4: Fairness Definitions and Conflicts
- Table 5.5: State-Level AI Regulations
- Table 5.6: Key Regulatory Trends
- Table 5.7: Fairness Responsibility Matrix
- Table 5.8: ML Lifecycle Stage Gates
- Table 5.9: Organizational Health Metrics
- Table 5.10: Key Terms: Conclusion

---

## Appendices

### Appendix A: Fairness Metrics Quick Reference
| Section | Page |
|---------|------|
| A.1 Metric Definitions at a Glance | ___ |
| A.2 Formulas | ___ |
| A.3 Interpretation Guide | ___ |
| A.4 Thresholds and Standards | ___ |
| A.5 Quick Decision Tree | ___ |
| A.6 Terminology Reference | ___ |
| A.7 Sample Size Guidelines | ___ |

### Appendix B: Regulatory Quick Reference
| Section | Page |
|---------|------|
| B.1 Overview: Key Regulations | ___ |
| B.2 Equal Credit Opportunity Act (ECOA) | ___ |
| B.3 Fair Credit Reporting Act (FCRA) | ___ |
| B.4 SR 11-7: Model Risk Management | ___ |
| B.5 EU AI Act | ___ |
| B.6 CFPB Guidance on AI | ___ |
| B.7 State-Level Regulations | ___ |
| B.8 Compliance Checklist | ___ |
| B.9 Key Contacts | ___ |

### Appendix C: Code Reference
| Section | Page |
|---------|------|
| C.1 Installation Requirements | ___ |
| C.2 Data Quality Functions | ___ |
| C.3 Fairness Metrics Functions | ___ |
| C.4 Bias Mitigation Functions | ___ |
| C.5 SHAP Explanation Functions | ___ |
| C.6 Monitoring Functions | ___ |
| C.7 Complete Example Script | ___ |

### Appendix D: Documentation Templates
| Section | Page |
|---------|------|
| D.1 Model Card Template | ___ |
| D.2 Fairness Testing Report Template | ___ |
| D.3 Production Monitoring Checklist | ___ |
| D.4 Pre-Deployment Sign-Off Form | ___ |
| D.5 Incident Response Template | ___ |

---

## Summary Statistics

| Component | Count |
|-----------|-------|
| **Chapters** | 5 |
| **Appendices** | 4 |
| **Figures** | 20 |
| **Tables** | 44 |

---

## Reading Paths

### Path A: Hands-On Practitioner (Full Journey)
**Time:** 8-12 hours
```
Chapter 1 → Chapter 2 → Chapter 3 → Chapter 4 → Chapter 5
   ↓           ↓           ↓           ↓
 Setup      Build       Model      Fairness    Apply
           Pipeline     + Fix      Testing
```
*Run all code, complete exercises, build the full pipeline*

### Path B: Technical Leader (Conceptual Focus)
**Time:** 3-4 hours
```
Chapter 1 → Chapter 3 (Key Takeaways) → Chapter 4 (4.1, 4.3) → Chapter 5
```
*Focus on concepts, skim code, understand trade-offs*

### Path C: Compliance/Risk Professional
**Time:** 2-3 hours
```
Chapter 1 → Chapter 4 → Chapter 5 → Appendix B → Appendix D
```
*Focus on regulatory requirements, documentation, governance*

### Path D: Quick Reference
**Time:** As needed
```
Appendix A (Metrics) + Appendix B (Regulations) + Appendix C (Code)
```
*Use as desk reference during projects*

---

## Quick Navigation

| I need to... | Go to... |
|--------------|----------|
| Understand why fairness matters | Chapter 1 |
| Build a data pipeline | Chapter 2 |
| Train a credit model | Chapter 3, Sections 3.1-3.2 |
| Fix a failing model | Chapter 3, Sections 3.3-3.4 |
| Explain model predictions | Chapter 3, Section 3.5 |
| Define fairness metrics | Chapter 4, Section 4.1 |
| Measure bias | Chapter 4, Section 4.2 |
| Mitigate bias | Chapter 4, Section 4.3 |
| Set up monitoring | Chapter 4, Section 4.4 |
| Document for regulators | Chapter 4, Section 4.5 + Appendix D |
| Look up a metric formula | Appendix A |
| Check regulatory requirements | Appendix B |
| Copy-paste code | Appendix C |
| Fill out a template | Appendix D |

---

*End of Table of Contents*
