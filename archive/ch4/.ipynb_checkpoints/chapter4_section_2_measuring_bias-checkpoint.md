# Section 4.2: Measuring Bias in Our Model
...
We start by loading everything we built in Section 2: the trained XGBoost model, the feature list, and the train/validation/test datasets. This allows us to generate predictions and analyze them for fairness.
...
# Important: We fit the imputer on TRAINING data only, then apply to val/test
# This prevents data leakage - we use training medians to fill val/test missing values
from sklearn.impute import SimpleImputer
imputer = SimpleImputer(strategy='median')
imputer.fit(X_train)  # Learn medians from training data
...
y_prob_val = best_model_obj.predict_proba(X_val)[:, 1]   # Probability of default
...
y_pred_val = (y_prob_val >= threshold).astype(int)   # 1 = predict default (deny)
y_pred_test = (y_prob_test >= threshold).astype(int) # 0 = predict no default (approve)
...
Let's implement all the fairness metrics from Section 3.1. Each function includes a brief description of what it measures and how to interpret the results.
...
def equal_opportunity_difference(y_true, y_pred, protected_attr, unprivileged_value, privileged_value):
    ...
    Where TPR = TP / (TP + FN) = fraction of actual defaults we correctly identified
    ...
    # Create masks to filter data to each group
    unprivileged_mask = (protected_attr == unprivileged_value)
    privileged_mask = (protected_attr == privileged_value)
    
    # Calculate TPR for each group separately
...
def average_odds_difference(y_true, y_pred, protected_attr, unprivileged_value, privileged_value):
    ...
    - TPR difference: Are we catching defaults at equal rates?
    - FPR difference: Are we wrongly denying good borrowers at equal rates?
    
    Uses absolute values, so direction doesn't matter - ANY disparity is flagged.
...
def expected_calibration_error(y_true, y_prob, n_bins=10):
    ...
    When model says "15% risk", do ~15% of those people actually default?
    
    ECE = Σ (bin_weight × |predicted_rate - actual_rate|)
    
    For each probability bin (0-10%, 10-20%, etc.):
    1. Calculate mean predicted probability in that bin
    2. Calculate actual default rate in that bin
    3. Weight the difference by fraction of data in that bin
    ...
    bin_indices = np.clip(bin_indices, 0, n_bins - 1)  # Handle edge case: prob = 1.0
    
    ece = 0
    for i in range(n_bins):
        in_bin = (bin_indices == i)  # Boolean mask for predictions in this bin
...
def get_group_metrics(y_true, y_pred, y_prob, protected_attr, group_value):
    ...
    Filters data to one group, then calculates comprehensive metrics.
    Works for any dataset (train, validation, or test).
    
    Returns dictionary with: n, default_rate, approval_rate, 
    TP/FP/TN/FN, TPR/FPR/TNR/FNR, precision, recall, mean_prob, ECE
    """
    ...
    # Basic counts from confusion matrix
...
Calibration measures whether the model's predictions are "honest" - does 15% predicted risk actually mean 15% default? Let's check if calibration differs across groups.
...
def calibration_by_group(y_true, y_prob, protected_attr, group_values, n_bins=5):
    ...
    For each group and probability bin, compares:
    - What the model predicted (mean probability)
    - What actually happened (actual default rate)
    """
    results = []
    ...
    for group in group_values:
        ...
        for bin_min, bin_max in [(0.0, 0.1), (0.1, 0.2), (0.2, 0.3), (0.3, 0.5), (0.5, 1.0)]:
            ...
            if in_bin.sum() > 0:
                predicted = y_prob_g[in_bin].mean()  # Mean predicted probability
                actual = y_true_g[in_bin].mean()     # Actual default rate
                count = in_bin.sum()
...
if len(cal_race_val) > 0:
    cal_race_val_display = cal_race_val.copy()
    cal_race_val_display['Predicted'] = cal_race_val_display['Predicted'].apply(lambda x: f'{x:.1%}')
    cal_race_val_display['Actual'] = cal_race_val_display['Actual'].apply(lambda x: f'{x:.1%}')
    cal_race_val_display['Error'] = cal_race_val_display['Error'].apply(lambda x: f'{x:.1%}')
    print(cal_race_val_display.to_string(index=False))
...
if len(cal_race_test) > 0:
    cal_race_test_display = cal_race_test.copy()
    cal_race_test_display['Predicted'] = cal_race_test_display['Predicted'].apply(lambda x: f'{x:.1%}')
    cal_race_test_display['Actual'] = cal_race_test_display['Actual'].apply(lambda x: f'{x:.1%}')
    cal_race_test_display['Error'] = cal_race_test_display['Error'].apply(lambda x: f'{x:.1%}')
    print(cal_race_test_display.to_string(index=False))

# Summary: ECE by group
print("\n\nExpected Calibration Error (ECE) by Group:")
print("-"*70)
print("ECE measures how 'honest' predictions are: does 15% predicted = 15% actual?")
print("\nValidation Set:")
for race in ['White', 'Black', 'Hispanic', 'Asian']:
...
def generate_fairness_report(dataset_name, y_true, y_pred, y_prob, race, gender, threshold):
    """
    Generate a comprehensive fairness report organized by tier.
...
This is why Section 3.3 will explore bias mitigation techniques:
...
Before moving to Section 3.3 (Bias Mitigation & Production), ensure you understand:
...
**Section 3.3 will address how to MITIGATE these biases and prepare models for production deployment.**
