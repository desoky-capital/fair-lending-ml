...
**What to do instead:**
- Use class weights in the loss function (e.g., `scale_pos_weight` in XGBoost) instead of synthetic oversampling
...
**Why it happened (illustrative example):**
```
Threshold: 0.25
...
Same model, same threshold, but the model "sees" test defaulters differently.
```
...
```
White, Default:     1.029  → Slightly under-represented (upweight 3%)
White, No Default:  0.999  → Balanced
Black, Default:     0.938  → Slightly over-represented (downweight 6%)
Black, No Default:  1.004  → Balanced
```

The model barely changed because the data was already reasonably balanced across groups.

**How to interpret weights:**
- Weight > 1: Under-represented → Model pays MORE attention
- Weight < 1: Over-represented → Model pays LESS attention  
- Weight ≈ 1: Balanced → No change needed
...
# If counts are similar across groups, reweighting won't help much
```
...
**The three main mitigation approaches act at different stages:**

```
          TRAINING              PREDICTION           DECISION
             │                      │                    │
             ▼                      ▼                    ▼
Data ──► [Reweighting] ──► Model ──► [Calibration] ──► Probs ──► [Thresholds] ──► Approve/Deny
         (adjust loss)              (adjust probs)              (adjust cutoff)
```

| Technique | When It Acts | What It Changes | Requires Retraining? |
|-----------|--------------|-----------------|---------------------|
| **Reweighting** | Before/during training | How model learns | Yes |
| **Calibration** | After training | Probability values | No |
| **Group Thresholds** | After training | Decision boundary | No |

**Quick definitions:**
- **Reweighting:** "Make underrepresented groups louder during training"
- **Calibration:** "Fix probabilities so 15% actually means 15%"
- **Group Thresholds:** "Use different cutoffs for different groups"
...
6. **Know when mitigations act** - Reweighting (training), Calibration (probabilities), Thresholds (decisions)

### Process
7. **Train/Validate/Test have distinct purposes** - Never tune on test
8. **Check fairness on multiple datasets** - Validation ≠ test ≠ production
9. **Document trade-offs** - Regulators want reasoning, not just decisions
10. **Monitor predictions, not just outcomes** - Catch problems early

### Conceptual
11. **Fairness definitions conflict** - Choose deliberately and document
12. **Fair but useless = useless** - Fairness is a constraint, not the objective
13. **Fairness is ongoing** - Monitor and re-evaluate continuously
14. **Technical fairness ≠ actual fairness** - Metrics are the floor, not the ceiling
